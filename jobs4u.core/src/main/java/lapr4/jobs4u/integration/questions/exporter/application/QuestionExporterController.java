...
public class QuestionExporterController {
    ...
    private final ListQuestionsService listSvc = new ListQuestionsService(
    ...
    public void exportInterview(final String filename, final FileFormat format, final QuestionImporterPlugin plugin)
            throws IOException {
        ...
        // Note that we are fetching all the data from the database. This of course will
        // only work
        // for a small amount of data. To export a large volume of data we should be
        // using some kind
    ...
    public void exportRequirements(final String filename, final FileFormat format, final QuestionImporterPlugin plugin)
            throws IOException {
        final QuestionExporter exporter = builder.build(format);
        authz.ensureAuthenticatedUserHasAnyOf(BaseRoles.POWERUSER, BaseRoles.CUSTOMER_MANAGER, BaseRoles.OPERATOR);
